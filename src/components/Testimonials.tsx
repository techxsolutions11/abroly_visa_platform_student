...
const Testimonials = () => {
    ...
    const initTestimonials = async () => {
        const { data, success } = await commonPublicGetApiCalls("/testimonials/list")
            ...
            <Swiper
                ...
                breakpoints={{
                    ...
                    640: {  // For screens larger than 640px (e.g., tablets and up)
                        slidesPerView: 2,  // 2 slides for wider screens
                    }
                }}
            >
                            ...
                            <blockquote className="text-center lg:mx-auto lg:w-3/5 space-y-5">
                                <div className="mt-6 lg:mt-10">
                                    <p className="relative text-xl sm:text-2xl md:text-3xl md:leading-normal font-medium text-gray-800">
                                        <svg className="absolute top-0 start-0 transform -translate-x-8 -translate-y-8 size-8 text-primary text-opacity-25 sm:h-10 sm:w-10" width="10" height="10" viewBox="0 0 16 13" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                                        ...
                                        <span className="relative z-10 italic text-gray-800 dark:text-neutral-200">{item?.description}</span>
                                    </p>
                                </div>
                                {/* <Button variant='flat' color='warning' isIconOnly><FaPlayCircle /></Button> */}
                                {item?.access_file && item?.access_file !== "" &&
                                    <video
                                        src={item?.access_file}
                                ...
                                <footer className="mt-6">
                                    <div className="font-semibold text-gray-800 dark:text-neutral-200">{item?.name}</div>
                                </footer>
                            </blockquote>
                        </div>
                    </SwiperSlide>
